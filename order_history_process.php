<?php
    ...
    //SQL文作成
    $sql = "SELECT * FROM order_list l,order_detail d,products p WHERE p.product_id = d.product_id AND l.order_id = d.order_id AND l.user_id=:user_id ORDER BY l.order_id desc";
    ...
    //order_id 取得
    $sql = "SELECT DISTINCT l.order_id,l.purchase_date,l.summary FROM order_list l,order_detail d,products p WHERE p.product_id = d.product_id AND l.order_id = d.order_id AND l.user_id=:user_id ORDER BY l.order_id desc";
    //$sql = "SELECT * FROM order_list WHERE user_id=:user_id";
    //プリペアードステートメントの設定と取得
    $prestmt = $pdo->prepare($sql);
    //値の設定
    ...
    foreach($order_id as $key => $row){
        ...
        $order_id[$key]['summary'] = number_format($row['summary']);
    }
...
<script>
    ...
    function init(){
        ...
        for(var row in order_id){
            ...
            div.textContent = "合計金額：￥"+order_id[row]['summary'];
            div_pa.appendChild(div);
