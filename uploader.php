...
if(isset($_POST['upload_mode'])){
    ...
    if($_POST['upload_mode'] == 'live_page'){
        ...
        if(isset($_POST['live_key'])){
            ...
            //SQL文作成
            $sql = "SELECT count(*)+1 as l_id FROM live;";
            //プリペアードステートメントの設定と取得
        ...
        for($i=0;$i<10;$i++){
            ...
            switch($flag){
                case 0:
                    $live_key .=chr(mt_rand(97, 122));
                    break;
                case 1:
                    $live_key .=chr(mt_rand(65, 90));
                    break;
                case 2:
                    $live_key .=chr(mt_rand(48, 57));
                    break;
                default:
                    $live_key .=chr(mt_rand(97, 122));
            }
        ...
        try{
            ...
            if (is_uploaded_file($tempthumbnail)) { 
                if ( move_uploaded_file($tempthumbnail , $thumbnail_name )) {
                    $sql = "INSERT INTO live (user_id,product_id,title,start_date,end_date,thumbnail,sum_live_tip,description,live_key) VALUES ('$user_ID',default,'$title', CURRENT_TIMESTAMP() ,default,'$thumbnail_name',default,'$description','$live_key')";  
                }
                else{
                    $sql = "INSERT INTO live (user_id,product_id,title,start_date,end_date,thumbnail,sum_live_tip,description,live_key) VALUES ('$user_ID',default,'$title', CURRENT_TIMESTAMP() ,default,default,default,'$description','$live_key')";  
                }
            }
            else{
                $sql = "INSERT INTO live (user_id,product_id,title,start_date,end_date,thumbnail,sum_live_tip,description,live_key) VALUES ('$user_ID',default,'$title', CURRENT_TIMESTAMP() ,default,default,default,'$description','$live_key')";  
